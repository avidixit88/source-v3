...
st.set_page_config(
    page_title="CAS Sourcing MVP v3",
    page_icon="🧪",
    layout="wide",
)

st.title("🧪 CAS Sourcing & Procurement Intelligence MVP v3")
st.caption(
    "CAS input → supplier discovery → visible pricing extraction → normalized price → bulk estimate → shortlist. "
    "v2 adds optional live supplier discovery while preserving v1 mock mode."
)
...
if run_search:
    ...
    if data_mode == "Stable mock data":
        ...
        with st.spinner("Running live supplier discovery and layered page extraction..."):
    ...
    if data_mode == "Live supplier discovery":
        st.caption(
            "Live mode now uses layered extraction: JSON-LD/meta tags, HTML tables, visible text windows, and supplier-page evidence snippets. It may still miss prices hidden behind account logins, heavy JavaScript, or quote gates. "
    ...
    st.download_button(
        ...
        data=csv,
        file_name=f"cas_supplier_shortlist_v2_{cas_number.replace('-', '_')}.csv",
        mime="text/csv",
    )
else:
